...
describe("StatTable", () => {
  ...
  it("keeps exactly one tab stop when rows shrinks past the previously-focused row", () => {
    const { container, rerender } = setup()
    const lastRowCell = screen.getByText("KC").closest("td")!
    act(() => {
      lastRowCell.focus()
    })
    expect(lastRowCell).toHaveAttribute("tabIndex", "0")
    ...
    const focusable = container.querySelectorAll('td[tabindex="0"]')
    expect(focusable).toHaveLength(1)
  })

  it("keeps sticky positioning on skeleton cells while loading", () => {
    setup({ isLoading: true })
    const skeletonRows = screen.getAllByTestId("stat-table-skeleton-row")
    const stickyCell = skeletonRows[0].querySelector("td")! // "team" is the sticky column
    expect(stickyCell).toHaveStyle({ position: "sticky", left: "0px" })
  })

  it("signs a fixed-precision value, keeping the leading zero on the signed branch", () => {
    type MarginRow = { team: string; margin: number }
    const marginColumns: StatColumn<MarginRow>[] = [
      { key: "team", label: "Team", width: 190, value: (r) => r.team },
      {
        key: "margin",
        label: "MARGIN",
  ...
  it("keeps a numeric column's header alignment the same whether rows are empty or populated", () => {
    const numericColumns: StatColumn<Row>[] = [
      { key: "team", label: "Team", width: 190, value: (r) => r.team },
      {
        key: "pct",
        label: "PCT",
    ...
    expect(screen.getByRole("columnheader", { name: "PCT" })).toHaveClass(
      "text-right",
    )
